...
window.nextStep = async function () {
  try {
    const cur = typeof step === 'number' ? step : 1; // foto del paso al inicio

    if (cur === 1) {
      if (typeof guardDetails === 'function' && !guardDetails()) return;

      const isLogged = (window.__pz_is_logged === true) ||
    ...
    if (cur === 3) {
      ...
      setStep(4);     // ¡sólo avanza!
      return;         // <- IMPORTANTÍSIMO: no seguir para no crear el plan aquí
    }

    if (cur === 4) {
      // Crear plan (sólo cuando pulsas otra vez en “Finalizar” o el botón morado)
      const fn =
